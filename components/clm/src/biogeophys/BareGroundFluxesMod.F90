module BareGroundFluxesMod
  ...
  use clm_instMod          , only : print_frictionvel_vars
  !
  ! !PUBLIC TYPES:
  implicit none
  !
...
contains
  ...
  !NOPE!  !$acc routine seq
    ...
    type(column_energy_flux), target, intent(inout) :: col_ef     ! column energy flux
    ...
    !------------------------------------------------------------------------------
      ...
      !!call print_frictionvel_vars(frictionvel_vars, bounds)
      ...
      do f = 1, fn
         ...
         print *, "Initialized MoninObukhov length and windspeed :",um(p),obu(p)
      ...
      do iter = 1, niters
         ...
         print *, "Calculating Friction Velocity from Bare Ground Fluxes"

         !$acc serial
         call FrictionVelocity(bounds%begp, bounds%endp, fn, filterp, &
              displa(bounds%begp:bounds%endp), z0mg_patch(bounds%begp:bounds%endp),&
               z0hg_patch(bounds%begp:bounds%endp), z0qg_patch(bounds%begp:bounds%endp), &
              obu(bounds%begp:bounds%endp), iter, ur(bounds%begp:bounds%endp), &
              um(bounds%begp:bounds%endp), ustar(bounds%begp:bounds%endp), &
              temp1(bounds%begp:bounds%endp), temp2(bounds%begp:bounds%endp),&
               temp12m(bounds%begp:bounds%endp), temp22m(bounds%begp:bounds%endp), fm(bounds%begp:bounds%endp), &
              frictionvel_vars)
          !$acc end serial
